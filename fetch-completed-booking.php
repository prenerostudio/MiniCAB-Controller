...
JOIN
    clients ON jobs.c_id = clients.c_id
JOIN
    drivers ON jobs.d_id = drivers.d_id
JOIN
    booking_type ON bookings.b_type_id = booking_type.b_type_id
JOIN
    vehicles ON bookings.v_id = vehicles.v_id
WHERE
    jobs.date_job_add >= NOW() - INTERVAL $selectedInterval HOUR
    AND jobs.date_job_add <= NOW()
