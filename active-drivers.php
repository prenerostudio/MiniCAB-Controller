    ...
    <div class="row row-deck row-cards">
        <div class="card">
            ...
            <div class="card-body border-bottom py-3">
                <div>
                    <table class="table table-responsive" id="table-driver">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>Image</th>
                        ...
                        <tbody class="table-tbody">											
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a href="javascript:void(0);" class="btn btn-danger btn-icon deleteDriverBtn" data-did="<?php echo $adrow['d_id']; ?>" title="Delete Driver">   
                                            <i class="ti ti-square-rounded-x"></i>
                                        </a>
                                    <a href="javascript:void(0);" 
   class="btn btn-icon btn-instagram makeInactiveBtn" 
   data-did="<?php echo $adrow['d_id']; ?>" 
   title="Make Inactive">
...
<script>   
    ...
    document.querySelectorAll('.deleteDriverBtn').forEach(function(button) {
        button.addEventListener('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    fetch('includes/drivers/del-driver.php', {
                        method: 'POST',
                        headers: {
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.makeInactiveBtn').forEach(function(button) {
        button.addEventListener('click', function() {
            const driverId = this.dataset.did;

            Swal.fire({
                title: 'Are you sure?',
                text: "You want to make this driver inactive?",
                icon: 'warning',
                showCancelButton: true,
                ...
                if (result.isConfirmed) {
                    ...
                    fetch('includes/drivers/make-inactive.php', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded'
                        },
                    ...
                    .then(data => {
                        ...
                        if (data.status === 'success') {
                            Swal.fire({
                                icon: 'success',
                                title: 'Driver Inactivated!',
                                text: data.message,
                                timer: 2000,
                                showConfirmButton: false
                            });

                            // Optionally fade out or update the row
                            const row = button.closest('tr');
