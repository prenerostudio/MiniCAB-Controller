...
if (isset($_POST['c_id'])) {
    $sql = "SELECT invoice.*, jobs.book_id, jobs.job_note, jobs.job_status, clients.c_name, clients.c_email, clients.c_phone, drivers.d_name, drivers.d_email, drivers.d_phone, bookings.b_type_id, bookings.pickup, bookings.stops, bookings.destination, bookings.passenger, bookings.pick_time, bookings.pick_date, bookings.journey_type, bookings.v_id, booking_type.*, SUM(invoice.total_pay) AS total_pay_amount FROM invoice INNER JOIN jobs ON invoice.job_id = jobs.job_id INNER JOIN clients ON invoice.c_id = clients.c_id INNER JOIN drivers ON invoice.d_id = drivers.d_id INNER JOIN bookings ON jobs.book_id = bookings.book_id INNER JOIN booking_type ON bookings.b_type_id = booking_type.b_type_id WHERE invoice.invoice_date BETWEEN '$start_date' AND '$end_date' AND invoice.c_id = '$c_id' GROUP BY invoice.invoice_id;";
