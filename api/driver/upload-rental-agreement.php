...
if ($fetch && $fetch->num_rows > 0) {
    ...
    $query = "UPDATE `rental_agreement` SET `ra_num`='$ra_num',`ra_exp`='$ra_exp',`ra_img`='$ra_img',`ra_updated_at`='$date_update' WHERE `d_id` = '$d_id'";
} else {
    // Insert new record
    $query = "INSERT INTO `rental_agreement`(`d_id`, `ra_num`, `ra_exp`, `ra_img`, `ra_created_at`) VALUES ('$d_id','$ra_num','$ra_exp','$ra_img','$date_update')";
}
