...
if (isset($_POST['d_id']) && !empty($_POST['d_id'])) {
    $d_id = $_POST['d_id'];
    error_log("Received d_id: " . $d_id); // Log the received d_id
    ...
    $sql = "SELECT jobs.*, clients.c_name, clients.c_email, clients.c_phone, clients.c_address, drivers.d_name, drivers.d_email, drivers.d_phone, bookings.* 
            ...
            AND (DATE(bookings.pick_date) = ? AND bookings.pick_time >= ? 
                 OR DATE(bookings.pick_date) > ?) 
            ORDER BY bookings.pick_date ASC, bookings.pick_time ASC";
    ...
    if ($stmt = mysqli_prepare($connect, $sql)) {
        ...
        if ($result) {
            ...
            error_log("Fetched Data: " . print_r($output, true));
    ...
    error_log("d_id not set or is empty in POST request");
