...
if ($fetch && $fetch->num_rows > 0) {
    ...
    $query = "UPDATE `vehicle_pco` SET `vpco_num`='$vpco_num',`vpco_exp`='$vpco_exp', `vpco_exp_time`='$vpco_exp_time', `vpco_img`='$vpco_img',`vpco_updated_at`='$date_update' WHERE `d_id` = '$d_id'";
} else {
    // Insert new record
    $query = "INSERT INTO `vehicle_pco`( `d_id`, `vpco_num`, `vpco_exp`, `vpco_exp_time`, `vpco_img`, `vpco_created_at`) VALUES ('$d_id','$vpco_num','$vpco_exp', '$vpco_exp_time','$vpco_img','$date_update')";
}
