...
if ($fetch && $fetch->num_rows > 0) {
    ...
    $query = "UPDATE `pco_license` 
              SET `pl_number` = '$pl_number', 
                  `pl_exp` = '$pl_exp', 
				  `pl_exp_time` = '$pl_exp_time',
                  `pl_img` = '$pco', 
                  `pl_updated_at` = '$date_update' 
              WHERE `d_id` = '$d_id'";
} else {
    // Insert new record
    $query = "INSERT INTO `pco_license` (`d_id`, `pl_number`, `pl_exp`, `pl_exp_time`, `pl_img`, `pl_created_at`) 
              VALUES ('$d_id', '$pl_number', '$pl_exp', '$pl_exp_time', '$pco', '$date_update')";
}
