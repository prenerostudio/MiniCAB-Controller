...
echo '<tbody>';
$x = 0;
$query = "SELECT d.d_id, d.d_name, d.`status`, s.driver_online_at, jobs.dropoff_time FROM drivers AS d JOIN driver_sessions AS s ON d.d_id = s.d_id JOIN jobs ON d.d_id = jobs.d_id WHERE d.`status` = 'online' AND s.driver_offline_at IS NULL GROUP BY d.d_id";
