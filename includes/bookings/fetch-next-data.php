...
$sql = "SELECT bookings.*, clients.c_name, clients.c_email, clients.c_phone, booking_type.*, vehicles.* FROM bookings LEFT JOIN clients ON bookings.c_id = clients.c_id INNER JOIN booking_type ON bookings.b_type_id = booking_type.b_type_id LEFT JOIN vehicles ON bookings.v_id = vehicles.v_id WHERE CONCAT(bookings.pick_date, ' ', bookings.pick_time) BETWEEN '$currentDateTime' AND '$futureDateTime' AND bookings.booking_status <> 'Booked' ORDER BY bookings.book_id DESC";
