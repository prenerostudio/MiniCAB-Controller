					...
					<div id="table-ndriver" class="table-responsive">
						<table class="table" id="table-new">
							...
							<tbody class="table-tbody">                            
								...
								AND dd.pco_license = '' 
								AND dd.address_proof_1 = '' 
								AND dd.address_proof_2 = '' 
								AND dd.dvla_check_code = '' 
								AND dd.national_insurance = '' 
								AND dd.extra = ''
								) LEFT JOIN vehicle_documents vd ON d.d_id = vd.d_id AND (
								vd.log_book = '' 
								AND vd.mot_certificate = '' 
								AND vd.pco = '' 
								AND vd.insurance = '' 
								AND vd.road_tax = '' 
								AND vd.vehicle_picture_front = '' 
								AND vd.vehicle_picture_back = '' 
								AND vd.rental_agreement = '' 
								AND vd.insurance_schedule = '' 
								AND vd.extra = ''
								) WHERE dd.d_id IS NOT NULL AND vd.d_id IS NOT NULL");                            
								while ($ndrow = mysqli_fetch_array($ndsql)) :                                
								$y++;                            
								?>                                
								<tr>                                
